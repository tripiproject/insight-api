...
StatusController.prototype.getBlockchainInfo = function(callback) {
  this.node.services.tripid.getBlockchainInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getMiningInfo = function(callback) {
    return this.node.services.tripid.getMiningInfo(function(err, result) {
...
StatusController.prototype.getStakingInfo = function(callback) {
    return this.node.services.tripid.getStakingInfo(function(err, result) {
...
StatusController.prototype.getDgpInfo = function(req, res) {
  ...
  return this.node.services.tripid.getDgpInfo(function(err, result) {
...
StatusController.prototype.getLastBlockHash = function() {
  var hash = this.node.services.tripid.tiphash;
  return {
    syncTipHash: hash,
    lastblockhash: hash
  };
...
StatusController.prototype.getBestBlockHash = function(callback) {
  this.node.services.tripid.getBestBlockHash(function(err, hash) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function(callback) {
  this.node.services.tripid.getBlockchainInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.sync = function(req, res) {
  ...
  this.node.services.tripid.isSynced(function(err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.tripid.syncPercentage(function(err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.tripid.height,
        syncPercentage: Math.round(percentage),
        height: self.node.services.tripid.height,
        error: null,
        type: 'tripicore node'
      };
